Shader "Hidden/Jeremy/UberPost"
{
    HLSLINCLUDE
        ...
        #pragma multi_compile_fragment _ _KL_BLACK_WHITE_FLASH
        ...
        half4 FragUberPost(Varyings input) : SV_Target
        {
            ...
            #if _KL_RADIAL_BLUR
            {
                color = ApplyRadialBlur(uvDistorted, TEXTURE2D_ARGS(_SourceTex, sampler_LinearClamp));
            }
            #endif

            #if _KL_PIXELIZE_QUAD
            {
                color = ApplyPixelizeQuad(uvDistorted, TEXTURE2D_ARGS(_SourceTex, sampler_LinearClamp));
            }
            #endif

            #if _KL_RGB_SPLIT
            {
                color = ApplyRGBSplit(color, uvDistorted, TEXTURE2D_ARGS(_SourceTex, sampler_LinearClamp));
            }
            #elif _KL_IMAGE_BLOCK
            {
                color = ApplyImageBlock(color, uvDistorted, TEXTURE2D_ARGS(_SourceTex, sampler_LinearClamp));
            }
            #elif _KL_ADVANCED_IMAGE_BLOCK
            {
                color = ApplyAdvancedImageBlock(color, uvDistorted, TEXTURE2D_ARGS(_SourceTex, sampler_LinearClamp));
            }
            #endif

            #if _KL_BLACK_WHITE_FLASH
            {
                color = ApplyBlackWhiteFlash(color, uvDistorted);
            }
            #endif

            #if _KL_SCANLINE
            {
                color = ApplyScanline(color, uvDistorted);
            }
            #endif
