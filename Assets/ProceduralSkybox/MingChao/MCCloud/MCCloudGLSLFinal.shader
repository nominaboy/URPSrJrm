Shader "MingChao/MCCloudGLSLFinal" {
    ...
    vertexshader {
            ...
            layout(location = 0) in vec4 _4;         //positionOS
            //layout(location = 1) in vec3 _7;         //tangentOS
            layout(location = 2) in vec4 _8;         //normalOS
            layout(location = 4) in vec4 _9;         //uvGroup
            layout(location = 0) out vec4 _12;       //normalWS
            layout(location = 1) out vec4 _13;       //uvGroup
            layout(location = 2) out vec4 _14;       //additionalColor
            layout(location = 3) out vec4 _15;       //positionWS
            //vec4 _11;
            vec4 _33;
            ...
            void main()
            {
                ...
                //vec3 _147 = cross(cross(_8.xyz, _7) * _8.www, _8.xyz) * _8.www;
                ...
                _164[0u] = _165[0u] * _25._m1[4u].xxx;
                _164[1u] *= _25._m1[4u].yyy;
                _164[2u] *= _25._m1[4u].zzz;
                //vec3 _200 = (_147.zzz * _164[2u]) + ((_147.yyy * _164[1u]) + (_147.xxx * _164[0u]));
                vec3 _201 = (_8.zzz * _164[2u]) + ((_8.yyy * _164[1u]) + (_8.xxx * _164[0u]));
                float _146 = _8.w * _25._m1[4u].w;
                ...
                vec3 _254 = (_4.xyz * _25._m1[6u].xyz) + _25._m1[5u].xyz;
                _242 = vec4(_254.x, _254.y, _254.z, _242.w);
                vec4 _257;
                _257.w = 1.0;
                ...
                _257 = vec4(_286.x, _286.y, _286.z, _257.w);
                vec4 _145 = _257;
                _145 = vec4(_257.xyz.x, _257.xyz.y, _257.xyz.z, _145.w); // positionWS + _PositionWSOffset
                ...
                if (_131 > 0.0)
                {
                    //_child0[5]	0.00, 0.00, 0.00, 0.00
                    vec3 _323 = _145.xyz + (-_134);

                    float _331 = min(_138.z, min(3.4028230607370965250836333559001e+38/*FLT_MAX*/, _138.z + 65536.0));
                    vec3 _330 = _323;
                    _330.z = _323.z + (_138.z + (-_331));//不变

                    float _351 = dot(_330, _330);
                    float _355 = inversesqrt(_351);
                    float _358 = _351 * _355;// |_330|
                    ...
                    {
                        vec3 _368 = _330 * vec3(_355);// normalize
                        float _373 = 1.0 + (-_100);//1.0 - 1.0
                        float _377 = step(_358, 6000000.0);
                        float _381 = mix(_107 / 1000.0, _110 / 1000.0, clamp((_358 + (-2000000.0)) / (-800000.0), 0.0, 1.0));
                        float _394 = max(_104, 0.01) * _355;
                        float _400 = _394 * _330.z;
                        float _405 = 1.0 + (-_394);
                        float _409 = ((_405 * _405) * _405) * _358;
                        float _417 = _358 + (-_122);
                        float _422 = (_114 * exp2(-max(-127.0, _381 * ((_331 + _400) + (-_138.z))))) / 1000.0;
                        float _440 = max(-127.0, _381 * (_330.z + (-_400)));
                        float _449 = (1.0 + (-exp2(-_440))) / _440;
                        float _457 = abs(_440);
                        float _460 = (_457 > 0.01) ? _449 : 0.693147182464599609375;
                        float _439 = _422 * _460;
                        float _469 = (dot(_64, _368) * 0.5) + 0.5;//_64 _child2[20]	-0.00119, -0.73807, 0.67472, -7.06947  lightdir?

                        float _476 = (min(pow(exp((_469 * 37.51020050048828125) + (-37.51020050048828125)), 
                            mix(_125, abs(dot(_368, vec3(0.0, 0.0, 1.0))) * _125, _118)), 1.0) * 
                    ...
                    _144 = mix(vec4(0.0, 0.0, 0.0, 1.0), _329, vec4(_131));
                }
                ...
                vec4 _619[1];
                _619[0u] = vec4(0.0);
                _619[0u] = vec4(_9.xy.x, _9.xy.y, _619[0u].z, _619[0u].w);
                vec4 _628[1];
                _628[0u] = _619[0u];
                _628[0u] = vec4(_628[0u].x, _628[0u].y, _9.zw.x, _9.zw.y);
                //vec4 _637;
                //_637.w = 0.0;
                //_637 = vec4(_200.x, _200.y, _200.z, _637.w);
                ...
                _12 = _642;//normalWS
                _13 = _628[0u];//uvGroup
                _14 = _144;//additionalColor
                _15 = _143;//positionWS.xyz(original) + (positionCS.w//FS没用到)
                vec4 _658 = _142;
                _41(_658);
                _142 = _658;
                gl_Position = _142;
    ...
    fragmentShader {
        ...
        layout(location = 2) in vec4 _15;//additionalColor
        ...
        void main()
        {
            ...
            vec3 _85 = _76 / vec3(_86);//normallize vector on xy-plane
            vec2 _95;
            _95.x = dot(_85, _70);
            ...
            vec3 _238 = _27._m2[15u].zzz * (
                    (
                        (
                            (
                                vec3(step(1.0 + (-(_27._m2[3u].y * _27._m3[2u].x)), dot(_27._m2[13u].xy, _145)) * 
                                dot(_27._m2[13u].xy, _153)) 
                                
                                * _27._m3[0u].xyz
                            ) * 

                                vec3(clamp((cloudTex.x + (-_27._m3[3u].y)) / _27._m3[3u].w, 0.0, 1.0))
            ...
            _571.w = clamp(
                    ...
                    (
                        (
                            ...
                            smoothstep(0.0, 200000.0, _61.z) * clamp(mix(cloudTex.w, mix(cloudTex.z * smoothstep(clamp(_567 + (-0.5), 0.0, 1.0), clamp(_567, 0.0, 1.0), 
                            texture(redMask, _218, _50).x), cloudTex.z, smoothstep(0.95, 1.0, _558)), smoothstep(-0.5, -0.3, _558)), 0.0, 1.0)

                        ) * _27._m2[1u].z
                    ) * 
                        clamp(
                                smoothstep(0.0, 1.0, cloudTex.x + _27._m3[6u].y)
                            , 0.0, 1.0), 0.0, 1.0
